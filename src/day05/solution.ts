...
const isConnectedHorizontally = ({ start, end }: LineEnds) => start.y === end.y
const isConnectedVertically = ({ start, end }: LineEnds) => start.x === end.x
  ...
  F.pipe(
    ...
    A.map(
      F.flow(
        ...
        A.filter((s) => s != " -> "),
        // Convert the [x,y] string tuple to numbers
...
export const pointsBetweenLineEnds = (lineEnds: LineEnds) => {
  const { start, end } = lineEnds
  ...
  if (isConnectedVertically(lineEnds)) {
  ...
  } else if (isConnectedHorizontally(lineEnds)) {
